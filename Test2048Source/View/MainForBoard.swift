...
class MainForBoard: UIView {
    let contentInset: CGFloat = 10
    var size = 4 {
        didSet {
            didUpdatePlaceholdLayer()
        }
    }
    var tiles = [MainForTitleView]()
    var placeholderLayers = [CALayer]()
    var colorScheme: [String : [String : String]] = {
        let data = try! Data(contentsOf: URL(fileURLWithPath: Bundle.main.path(forResource: "NumberColor", ofType: "json")!))
        do {
    ...
    func spawnTile(at position: BoardPosition) -> MainForTitleView {
        let tile = MainForTitleView(frame: frame(at: position))
        tiles.append(tile)
        addSubview(tile)
        tile.theme = colorScheme
    ...
    func titleAniFrame() {
        var destroyed = [MainForTitleView]()
        for tile in tiles {
            UIView.animate(withDuration: 0.1, animations: {
    ...
    fileprivate func didUpdatePlaceholdLayer() {
    ...
    fileprivate func tile(at position: BoardPosition) -> MainForTitleView? {
